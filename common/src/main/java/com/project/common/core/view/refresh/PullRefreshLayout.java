 ...
 * <li>可在 xml 中使用  这些属性或在 Java 设置对应的属性决定子View的开始位置、触发刷新的位置等值</li>
 * </ul>
 *
 * @author cginechen
 * @date 2016-12-11
...
public class PullRefreshLayout extends ViewGroup implements NestedScrollingParent {
    ...
    public PullRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.style.PullRefreshLayoutStyle);
    }
    ...
    public PullRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTouchSlop = (int) MeasureUtils.px2dp(context, mSystemTouchSlop); //系统的值是8dp,如何配置？
        ...
        try {
            mRefreshInitOffset = array.getDimensionPixelSize(
                    R.styleable.PullRefreshLayout_qmui_refresh_init_offset, Integer.MIN_VALUE);
            mRefreshEndOffset = array.getDimensionPixelSize(
                    R.styleable.PullRefreshLayout_qmui_refresh_end_offset, Integer.MIN_VALUE);
            mTargetInitOffset = array.getDimensionPixelSize(
                    R.styleable.PullRefreshLayout_qmui_target_init_offset, 0);
            mTargetRefreshOffset = array.getDimensionPixelSize(
                    R.styleable.PullRefreshLayout_qmui_target_refresh_offset,
                    (int) MeasureUtils.dp2px(getContext(), 72));
            mAutoCalculateRefreshInitOffset = mRefreshInitOffset == Integer.MIN_VALUE ||
                    array.getBoolean(R.styleable.PullRefreshLayout_qmui_auto_calculate_refresh_init_offset, false);
            mAutoCalculateRefreshEndOffset = mRefreshEndOffset == Integer.MIN_VALUE ||
                    array.getBoolean(R.styleable.PullRefreshLayout_qmui_auto_calculate_refresh_end_offset, false);
            mEqualTargetRefreshOffsetToRefreshViewHeight = array.getBoolean(R.styleable.PullRefreshLayout_qmui_equal_target_refresh_offset_to_refresh_view_height, false);
        } finally {
            array.recycle();
        }
    ...
    protected View createRefreshView() {
        return new FMLRefreshView(getContext());
    }
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mTargetCurrentOffset != mTargetInitOffset) {
                mScroller.startScroll(0, mTargetCurrentOffset, 0, mTargetInitOffset - mTargetCurrentOffset, 1000);
            }
            invalidate();
        } else if (hasFlag(FLAG_NEED_SCROLL_TO_REFRESH_POSITION)) {
            removeFlag(FLAG_NEED_SCROLL_TO_REFRESH_POSITION);
            if (mTargetCurrentOffset != mTargetRefreshOffset) {
                mScroller.startScroll(0, mTargetCurrentOffset, 0, mTargetRefreshOffset - mTargetCurrentOffset, 1000);
            } else {
...
//            mProgress.setColorSchemeColors(ResUtils.getAttrColor(context, R.attr.color_blue));
