...
public class PasswordInputView extends AppCompatEditText {
    ...
    private void getDefaultVar() {
        borderNotFocusedColor = getColor(getContext(), R.color.color1);
        borderFocusedColor = getColor(getContext(), R.color.theme_color);
    }
    ...
    private void initAttrVar(Context context, AttributeSet attrs) {
        ...
        dotRadius = ta.getDimension(R.styleable.PasswordInput_dotRaduis, DisplayUtil.dip2px(context, 11));
        ta.recycle();
        ...
        borderWidth = DisplayUtil.dip2px(context, 1);
        boxRadius = DisplayUtil.dip2px(context, 1);
        boxMarge = DisplayUtil.dip2px(context, 1);
    }
    /**
     * 获取颜色兼容
     *
...
//        }
        ...
        RectF rect = new RectF(DisplayUtil.dip2px(getContext(), 1), DisplayUtil.dip2px(getContext(), 1), width-DisplayUtil.dip2px(getContext(), 1), height);
//        canvas.drawRoundRect(rect, boxRadius, boxRadius, mBorderPaint);
        canvas.drawRect(rect,mBorderPaint);

        for (int i =1;i < boxCount;i++){
            float x = width * i / boxCount;
